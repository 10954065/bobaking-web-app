...
import { subscribeToDeliveryEvents } from "@/modules/delivery/services/delivery-events";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ branchId: string }> }) {
  ...
  if (!hasPermission(profile, "delivery", "read", branchId)) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      unsubscribe = subscribeToDeliveryEvents(branchId, (event) => {
